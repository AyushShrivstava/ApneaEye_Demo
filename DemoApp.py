...
video_writer = cv2.VideoWriter("Data/demo_record{}.avi".format(timestamp), cv2.VideoWriter_fourcc(*'XVID'), 25, (256, 192))
...
while True:
    ...
    if box.shape[0] > 0:
        ...
        # check if chest is detected. If detected, take the biggest chest. If not, take the previous value
        if (box_df['class'] == 0).any():
            biggest_chest_idx = box_df[box_df['class'] == 0]['area'].idxmax()
            chest = box_df.loc[biggest_chest_idx]

            # cv2.rectangle(imdata, (int(chest['x1']), int(chest['y1'])),
            #                 (int(chest['x2']), int(chest['y2'])), (0, 255, 0), 2)
    ...
    if chest is not None:
        ...
        edges[:,:int(chest['x1'])] = 0
        edges[:,int(chest['x2']):] = 0
        edges[:int(chest['y1'])] = 0
        edges[int(chest['y2']):] = 0
        ...
        cv2.rectangle(imdata, (int(chest['x1']), int(chest['y1'])),
                            (int(chest['x2']), int(chest['y2'])), (255, 0, 0), 2)
